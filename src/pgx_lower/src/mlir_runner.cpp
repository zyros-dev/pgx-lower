...
bool run_mlir_with_direct_data_access(void* dataPtr, size_t dataSize,
                                      MLIRLogger& logger) {
    DataAccessContext dataContext = {dataPtr, dataSize, sizeof(int64_t)};
    g_data_context = &dataContext;

    mlir::MLIRContext context;

    std::ostringstream oss;
    oss << "Processing " << dataSize << " bytes of data directly in MLIR";
    logger.debug(oss.str());
    ...
    auto dataSizeCall = builder.create<mlir::func::CallOp>(loc, dataSizeFunc,
                                                           mlir::ValueRange{});
    mlir::Value dataSizeValue = dataSizeCall.getResult(0);

    mlir::Value sum = zeroConst;

    for (int i = 0; i < 5; i++) {
        auto offsetConst = builder.create<mlir::arith::ConstantOp>(
            loc, builder.getI64IntegerAttr(i * 8));

        auto readCall = builder.create<mlir::func::CallOp>(
            loc, memReadFunc, mlir::ValueRange{ptrConst, offsetConst});
        mlir::Value value = readCall.getResult(0);

        auto newSum = builder.create<mlir::arith::AddIOp>(loc, sum, value);
        sum = newSum.getResult();
    }

...
struct AggregateSpec {
    ...
    Expr* input_expr;
    Oid result_type_oid;
    mlir::Type result_mlir_type;

    std::string column_name;

    enum Source { TARGETLIST, HAVING, NESTED_IN_EXPR };
    Source source;
    int targetlist_resno;
};
...
struct PostProcessExpr {
    int resno;
    Expr* full_expr;
    std::set<int> used_aggnos;
};

struct AggregationAnalysis {
    std::vector<GroupByColumn> group_by_columns;
    std::vector<AggregateSpec> aggregate_specs;
    std::vector<PostProcessExpr> post_process_exprs;


    std::map<int, size_t> aggno_to_index;
};


struct AggregationBuildResult {
    mlir::Value aggregation_op;
    std::string aggregate_scope;
    std::map<int, std::pair<std::string, std::string>> mappings;
};
...
/**
 * Stage 1: Analyze the aggregation to understand what needs to be computed
 *
 * Inputs:
 *   - agg: PostgreSQL Agg plan node
 *   - childResult: Result from translating the child plan (IMMUTABLE)
 *
 * Outputs:
 *   - AggregationAnalysis containing all specs for building the aggregation
 *
 * This function is PURE - no side effects, no mutations
 */
auto analyze_aggregation(
    const Agg* agg,
    const TranslationResult& childResult,
    mlir::OpBuilder& builder
) -> AggregationAnalysis {
    AggregationAnalysis analysis;

    // TODO: Implementation
    // 1. Extract GROUP BY columns from grpColIdx and targetlist
    // 2. Find all Aggref nodes in targetlist and HAVING
    // 3. Build AggregateSpec for each unique aggregate
    // 4. Identify expressions needing post-processing
    // 5. Build aggno_to_index map

    return analysis;
}

// ==================== Stage 2: Build Aggregation ====================

/**
 * Stage 2: Build the MLIR AggregationOp based on the analysis
 *
 * Inputs:
 *   - ctx: Query context for builder and type information
 *   - childOutput: The MLIR Value representing the child's output stream
 *   - analysis: The aggregation analysis from Stage 1
 *
 * Outputs:
 *   - AggregationBuildResult containing the operation and mappings
 *
 * Creates the AggregationOp with proper GROUP BY and aggregate computations
 */
auto build_aggregation_op(
    QueryCtxT& ctx,
    mlir::Value childOutput,
    const AggregationAnalysis& analysis
) -> AggregationBuildResult {
    AggregationBuildResult result;

    // TODO: Implementation
    // 1. Create aggregate scope name
    // 2. Build GROUP BY attribute array
    // 3. Create aggregate function block
    // 4. For each AggregateSpec, create the appropriate MLIR operations
    // 5. Create AggregationOp
    // 6. Build mappings
...
/**
 * Stage 3: Apply post-processing for complex expressions over aggregates
 *
 * Inputs:
 *   - ctx: Query context
 *   - buildResult: Result from building the aggregation
 *   - postProcessExprs: Expressions that need post-processing
 *
 * Outputs:
 *   - PostProcessResult with final operation and scope
 *
 * Creates a MapOp if needed for expressions like SUM(x) * COUNT(y)
 */
auto apply_post_processing(
    QueryCtxT& ctx,
    const AggregationBuildResult& buildResult,
    const std::vector<PostProcessExpr>& postProcessExprs
) -> PostProcessResult {
    PostProcessResult result;

    if (postProcessExprs.empty()) {
        // No post-processing needed
        result.final_op = buildResult.aggregation_op;
        result.final_scope = buildResult.aggregate_scope;
        return result;
    }
    ...
    // 1. Create MapOp
    // 2. Create map block
    // 3. For each PostProcessExpr, translate the expression
    // 4. Return the MapOp and new scope

    return result;
}

// ==================== Stage 4: Build Translation Result ====================

/**
 * Stage 4: Build the complete TranslationResult
 *
 * Inputs:
 *   - targetlist: PostgreSQL targetlist (output columns specification)
 *   - childResult: Child's result (for GROUP BY column info)
 *   - analysis: Aggregation analysis
 *   - buildResult: Aggregation build result
 *   - postResult: Post-processing result
 *
 * Outputs:
 *   - Complete TranslationResult with operation, columns, scope, and mappings
 *
 * Assembles all components into the final result structure
 */
auto build_translation_result(
    const List* targetlist,
    const TranslationResult& childResult,
    const AggregationAnalysis& analysis,
    const AggregationBuildResult& buildResult,
    const PostProcessResult& postResult
) -> TranslationResult {
    TranslationResult result;

    // Set the operation and scope
    result.op = postResult.final_op.getDefiningOp();
    result.current_scope = postResult.final_scope;

    // Build output columns
    // TODO: Implementation
    // 1. Iterate through targetlist
    // 2. For each entry, determine if it's:
    //    - An aggregate (use buildResult.mappings)
    //    - A GROUP BY column (use childResult)
    //    - A post-processed expression (use postResult.final_scope)
    // 3. Build appropriate ColumnSchema and add to result.columns

    // Add aggregate mappings to varno_resolution for HAVING clause
    for (const auto& [aggno, mapping] : buildResult.mappings) {
...
auto PostgreSQLASTTranslator::Impl::translate_agg(QueryCtxT& ctx, const Agg* agg) -> TranslationResult {
    ...
    if (!agg || !agg->plan.lefttree) {
        throw std::runtime_error("Invalid Agg node: missing required fields");
    }

    const auto childResult = translate_plan_node(ctx, agg->plan.lefttree);
    if (!childResult.op || !childResult.op->getNumResults()) {
        throw std::runtime_error("Failed to translate Agg child plan");
    }
    const auto childOutput = childResult.op->getResult(0);

    auto analysis = analyze_aggregation(agg, childResult, ctx.builder);
    auto buildResult = build_aggregation_op(ctx, childOutput, analysis);
    auto postResult = apply_post_processing(ctx, buildResult, analysis.post_process_exprs);
    auto result = build_translation_result(agg->plan.targetlist, childResult, analysis, buildResult, postResult);

    if (agg->plan.qual && agg->plan.qual->length > 0) {
        PGX_LOG(AST_TRANSLATE, DEBUG, "Agg: Stage 5 - Applying HAVING clause");
...
auto PostgreSQLASTTranslator::Impl::translate_agg_old(QueryCtxT& ctx, const Agg* agg) -> TranslationResult {
    PGX_IO(AST_TRANSLATE);
    if (!agg || !agg->plan.lefttree)

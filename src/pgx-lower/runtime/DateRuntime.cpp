...
static inline DateADT nanos_to_postgres_date(int64_t nanos) {
    constexpr int64_t NANOS_PER_DAY = 86400000000000LL;
    int64_t const days = nanos / NANOS_PER_DAY;
    return static_cast<DateADT>(days);
}

static inline int64_t postgres_timestamp_to_nanos(Timestamp ts) {
    constexpr int64_t POSTGRES_EPOCH_OFFSET_USEC = INT64CONST(946684800000000);
    int64_t const micros = ts + POSTGRES_EPOCH_OFFSET_USEC;
    return micros * 1000;
}
...
int64_t runtime::DateRuntime::extractYear(int64_t nanos) {
    ...
    DateADT const pg_date = nanos_to_postgres_date(nanos);
    PGX_LOG(RUNTIME, DEBUG, "converted to pg_date=%d", pg_date);

    int const jd = pg_date + POSTGRES_EPOCH_JDATE;
...
int64_t runtime::DateRuntime::ExtractFromDate(VarLen32 field, int64_t date) {
    uint32_t const len = field.getLen();
    char* const data = field.data();

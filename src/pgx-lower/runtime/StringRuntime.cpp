...
bool iterative_like(const char* str, size_t strLen, const char* pattern, size_t patternLen, char escape) {
   const char *s = str;
   ...
   for (; plen > 0 && slen > 0; NextByte(p, plen)) {
      if (*p == escape) {
         ...
         while (slen > 0) {
            if (iterative_like(s, slen, p, plen, escape)) {
               return true;
            }
            NextByte(s, slen);
         }
...
bool runtime::StringRuntime::like(runtime::VarLen32 str1, runtime::VarLen32 str2) {
   return iterative_like((str1).data(), (str1).getLen(), (str2).data(), (str2).getLen(), '\\');
}
...
int64_t runtime::StringRuntime::toInt(runtime::VarLen32 str) {
   ...
   std::string const temp_str(trimmed_data, trimmed_len);
   
   // Parse the integer
   char* endptr = nullptr;
   errno = 0;
   int64_t const val = strtoll(temp_str.c_str(), &endptr, 10);
   
   // Check for errors
   if (errno == ERANGE || endptr != temp_str.c_str() + temp_str.length()) {
      std::string const err = "Failed to cast the string " + std::string(data, len) + " to int64_t";
...
float runtime::StringRuntime::toFloat32(runtime::VarLen32 str) {
   ...
   char* endptr = nullptr;
   errno = 0;
   float const val = strtof(temp_str.c_str(), &endptr);
   
   // Check for errors
   if (errno == ERANGE || endptr != temp_str.c_str() + temp_str.length()) {
      std::string const err = "Failed to cast the string " + std::string(data, len) + " to float";
      throw std::runtime_error(err);
   }
...
double runtime::StringRuntime::toFloat64(runtime::VarLen32 str) {
   ...
   char* endptr = nullptr;
   errno = 0;
   double const val = strtod(temp_str.c_str(), &endptr);
   
   // Check for errors
   if (errno == ERANGE || endptr != temp_str.c_str() + temp_str.length()) {
      std::string const err = "Failed to cast the string " + std::string(data, len) + " to double";
      throw std::runtime_error(err);
   }
...
__int128 runtime::StringRuntime::toDecimal(runtime::VarLen32 string, int32_t req_scale) {
   std::string const str = string.str();

   PGX_LOG(RUNTIME, DEBUG, "StringRuntime::toDecimal: Converting string '%s' to decimal with reqScale=%d",
           str.c_str(), req_scale);

   // Find the decimal point
   size_t const decimal_pos = str.find('.');
...
runtime::VarLen32 runtime::StringRuntime::fromInt(int64_t value) {
   ...
   char buffer[32];
   int const len = snprintf(buffer, sizeof(buffer), "%lld", static_cast<long long>(value));
   
   if (len < 0 || len >= static_cast<int>(sizeof(buffer))) {
      throw std::runtime_error("Failed to convert int64_t to string");
   }
   
   uint8_t* const data = pgx_alloc(len);
...
runtime::VarLen32 runtime::StringRuntime::fromFloat32(float value) {
   ...
   char buffer[64];
   int const len = snprintf(buffer, sizeof(buffer), "%.9g", value);
...
runtime::VarLen32 runtime::StringRuntime::fromFloat64(double value) {
   ...
   char buffer[64];
   int const len = snprintf(buffer, sizeof(buffer), "%.17g", value);
...
runtime::VarLen32 runtime::StringRuntime::fromChar(uint64_t val, size_t bytes) {
   uint8_t* const data = pgx_alloc(bytes);
...
runtime::VarLen32 runtime::StringRuntime::substr(runtime::VarLen32 str, size_t from, size_t to) {
   ...
   return {&str.getPtr()[from], to - from};
}

size_t runtime::StringRuntime::findMatch(VarLen32 str, VarLen32 needle, size_t start, size_t end) {
   constexpr size_t INVALID_POS = 0x8000000000000000;
   if (start >= INVALID_POS) return INVALID_POS;
   if (start + needle.getLen() > end) return INVALID_POS;
   size_t const found = std::string_view(str.data(), str.getLen()).find(std::string_view(needle.data(), needle.getLen()), start);

   if (found == std::string::npos || found + needle.getLen() > end) return INVALID_POS;
...
runtime::VarLen32 runtime::StringRuntime::concat(runtime::VarLen32 left, runtime::VarLen32 right) {
   ...
   uint32_t const total_len = left.getLen() + right.getLen();
   uint8_t* const result = pgx_alloc(total_len);
...
runtime::VarLen32 runtime::StringRuntime::concat3(runtime::VarLen32 a, runtime::VarLen32 b, runtime::VarLen32 c) {
   uint32_t const total_len = a.getLen() + b.getLen() + c.getLen();
   uint8_t* const result = pgx_alloc(total_len);
...
runtime::VarLen32 runtime::StringRuntime::upper(runtime::VarLen32 str) {
   uint32_t const len = str.getLen();
   uint8_t* const result = pgx_alloc(len);
   
   for (uint32_t i = 0; i < len; i++) {
      char const ch = static_cast<char>(str.getPtr()[i]);
...
runtime::VarLen32 runtime::StringRuntime::substring(runtime::VarLen32 str, int32_t start, int32_t length) {
   ...
   int32_t const start_idx = start - 1;
   int32_t const str_len = static_cast<int32_t>(str.getLen());
   
   // Handle out of bounds
   if (start_idx >= str_len || length <= 0) {
      return {pgx_alloc(0), 0};
   }
   
   // Adjust length if it exceeds string length
   if (start_idx + length > str_len) {
      length = str_len - start_idx;
   }
   
   uint8_t* const result = pgx_alloc(length);
   memcpy(result, str.getPtr() + start_idx, length);
...
runtime::VarLen32 runtime::StringRuntime::trim(runtime::VarLen32 str) {
   const uint8_t* const data = str.getPtr();
   int32_t const len = str.getLen();
   ...
   int32_t const trimmed_len = end - start + 1;
   if (trimmed_len <= 0) {
      return {pgx_alloc(0), 0};
   }
   
   uint8_t* const result = pgx_alloc(trimmed_len);
   memcpy(result, data + start, trimmed_len);
   
   return {result, trimmed_len};
}
...
runtime::VarLen32 runtime::StringRuntime::ltrim(runtime::VarLen32 str) {
   ...
   int32_t const trimmed_len = len - start;
   if (trimmed_len <= 0) {
      return {pgx_alloc(0), 0};
   }
...
runtime::VarLen32 runtime::StringRuntime::rtrim(runtime::VarLen32 str) {
   ...
   int32_t const trimmed_len = end + 1;
   if (trimmed_len <= 0) {
      return {pgx_alloc(0), 0};
   }
   
   uint8_t* const result = pgx_alloc(trimmed_len);
   memcpy(result, data, trimmed_len);
...
bool runtime::StringRuntime::ilike(runtime::VarLen32 str, runtime::VarLen32 pattern) {
   runtime::VarLen32 const lower_str = lower(str);
   runtime::VarLen32 const lower_pattern = lower(pattern);
   
   bool const result = like(lower_str, lower_pattern);
   return result;
}

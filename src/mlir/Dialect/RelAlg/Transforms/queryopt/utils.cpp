...
Operator Plan::realizePlanRec() {
   ...
   if (!isLeaf) {
      if (currop) {
         ...
         ::mlir::OpBuilder builder2(currop->getContext());
         currop->setAttr("cost", ::mlir::FloatAttr::get(builder2.getF64Type(), cost));
         currop->setAttr("rows", ::mlir::FloatAttr::get(builder2.getF64Type(), rows));

...
class AtLowering : public OpConversionPattern<mlir::dsa::At> {
   ...
   LogicalResult matchAndRewrite(mlir::dsa::At atOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      } else if (auto dateType = t.dyn_cast<mlir::db::DateType>()) {
      ...
      auto newAtOp = rewriter.create<mlir::dsa::At>(loc, types, adaptor.getCollection(), atOp.pos());
      ...
      if (t.isa<mlir::db::DateType, mlir::db::TimestampType>()) {
         ...
         size_t multiplier = 1;
         if (auto dateType = t.dyn_cast<mlir::db::DateType>()) {
            multiplier = dateType.getUnit() == mlir::db::DateUnitAttr::day ? 86400000000000 : 1000000;
         } else if (auto timeStampType = t.dyn_cast<mlir::db::TimestampType>()) {
      ...
      } else if (auto decimalType = t.dyn_cast<db::DecimalType>()) {
...
class StringCastOpLowering : public OpConversionPattern<mlir::db::CastOp> {
   ...
   LogicalResult matchAndRewrite(mlir::db::CastOp castOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      if (scalarSourceType == scalarTargetType) {
         //nothing to do here
      } else if (auto stringType = scalarSourceType.dyn_cast<db::StringType>()) {
         ...
         } else if (auto floatType = scalarTargetType.dyn_cast<FloatType>()) {
         ...
         } else if (auto decimalType = scalarTargetType.dyn_cast<db::DecimalType>()) {
      ...
      } else if (auto floatType = scalarSourceType.dyn_cast<FloatType>()) {
      ...
      } else if (auto decimalSourceType = scalarSourceType.dyn_cast<db::DecimalType>()) {
      ...
      } else if (auto charType = scalarSourceType.dyn_cast<db::CharType>()) {
...
class BinOpLowering : public OpConversionPattern<OpClass> {
   ...
   LogicalResult matchAndRewrite(OpClass binOp, typename OpConversionPattern<OpClass>::OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      if (type.template isa<OperandType>()) {
         rewriter.template replaceOpWithNewOp<StdOpClass>(binOp, this->typeConverter->convertType(binOp.result().getType()), adaptor.getLeft(), adaptor.getRight());
         return success();
      }
      return failure();
   }
...
class ConstantLowering : public OpConversionPattern<mlir::db::ConstantOp> {
   ...
   LogicalResult matchAndRewrite(mlir::db::ConstantOp constantOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      if (auto integerAttr = constantOp.getValue().dyn_cast<IntegerAttr>()) {
         parseArg = integerAttr.getInt();
      } else if (auto floatAttr = constantOp.getValue().dyn_cast<FloatAttr>()) {
         parseArg = floatAttr.getValueAsDouble();
      } else if (auto stringAttr = constantOp.getValue().dyn_cast<StringAttr>()) {
         parseArg = stringAttr.str();
      } else {
         return failure();
      }
      auto parseResult = support::parse(parseArg, arrowType, param1, param2);
      if (auto intType = stdType.dyn_cast<IntegerType>()) {
         if (auto decimalType = type.dyn_cast<mlir::db::DecimalType>()) {
      ...
      } else if (auto floatType = stdType.dyn_cast<FloatType>()) {
   ...
   LogicalResult matchAndRewrite(mlir::db::CastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      if (auto sourceIntWidth = getIntegerWidth(scalarSourceType, false)) {
         if (scalarTargetType.isa<FloatType>()) {
            ...
            rewriter.replaceOp(op, value);
            return success();
         } else if (auto decimalTargetType = scalarTargetType.dyn_cast<db::DecimalType>()) {
         ...
         if (auto decimalTargetType = scalarTargetType.dyn_cast<db::DecimalType>()) {
...
class BetweenLowering : public OpConversionPattern<mlir::db::BetweenOp> {
   ...
   LogicalResult matchAndRewrite(mlir::db::BetweenOp betweenOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      auto isGteLower = rewriter.create<mlir::db::CmpOp>(betweenOp->getLoc(), betweenOp.lowerInclusive() ? mlir::db::DBCmpPredicate::gte : mlir::db::DBCmpPredicate::gt, betweenOp.getVal(), betweenOp.lower());
      auto isLteUpper = rewriter.create<mlir::db::CmpOp>(betweenOp->getLoc(), betweenOp.upperInclusive() ? mlir::db::DBCmpPredicate::lte : mlir::db::DBCmpPredicate::lt, betweenOp.getVal(), betweenOp.upper());
...
class HashLowering : public ConversionPattern {
   ...
   Value hashImpl(OpBuilder& builder, Location loc, Value v, Value totalHash, Type originalType) const {
      if (auto intType = v.getType().dyn_cast<mlir::IntegerType>()) {
      ...
      } else if (auto floatType = v.getType().dyn_cast<::mlir::FloatType>()) {
         assert(false && "can not hash float values");
      } else if (auto varLenType = v.getType().dyn_cast<mlir::util::VarLen32Type>()) {
         auto hash = builder.create<mlir::util::HashVarLen>(loc, builder.getIndexType(), v);
         return combineHashes(builder, loc, hash, totalHash);
      } else if (auto tupleType = v.getType().dyn_cast<mlir::TupleType>()) {
         if (auto originalTupleType = originalType.dyn_cast<mlir::TupleType>()) {

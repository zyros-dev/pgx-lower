...
std::vector<::mlir::Value> pgx::mlir::relalg::Translator::mergeRelationalBlock(::mlir::Block* dest, ::mlir::Operation* op, ::mlir::function_ref<::mlir::Block*(::mlir::Operation*)> getBlockFn, TranslatorContext& context, TranslatorContext::AttributeResolverScope& scope) {
   ...
   std::vector<::mlir::Value> res(returnOp.results().begin(), returnOp.results().end());
   terminator->erase();
   return res;
}

// Remove these invalid implementations that don't match the header.
// The header only defines mergeRelationalBlock and createTranslator.
// propagateInfo() is defined in the header as a protected method.

...
class SortTranslator : public pgx::mlir::relalg::Translator {
   ...
   virtual void consume(pgx::mlir::relalg::Translator* child, ::mlir::OpBuilder& builder, pgx::mlir::relalg::TranslatorContext& context) override {
      ...
      builder.create<pgx::mlir::dsa::Append>(sortOp->getLoc(), vector, packed);
   }
   ::mlir::Value createSortPredicate(::mlir::OpBuilder& builder, std::vector<std::pair<mlir::Value, mlir::Value>> sortCriteria, ::mlir::Value trueVal, ::mlir::Value falseVal, size_t pos) {
      if (pos < sortCriteria.size()) {
         ::mlir::Value lt = builder.create<pgx::mlir::db::CmpOp>(sortOp->getLoc(), pgx::mlir::db::DBCmpPredicate::lt, sortCriteria[pos].first, sortCriteria[pos].second);
         lt = builder.create<pgx::mlir::db::DeriveTruth>(sortOp->getLoc(), lt);
         auto ifOp = builder.create<::mlir::scf::IfOp>(
            sortOp->getLoc(), builder.getI1Type(), lt, [&](::mlir::OpBuilder& builder, ::mlir::Location loc) { builder.create<::mlir::scf::YieldOp>(loc, trueVal); }, [&](::mlir::OpBuilder& builder, ::mlir::Location loc) {
               ::mlir::Value eq = builder.create<pgx::mlir::db::CmpOp>(loc, pgx::mlir::db::DBCmpPredicate::eq, sortCriteria[pos].first, sortCriteria[pos].second);
               eq=builder.create<pgx::mlir::db::DeriveTruth>(sortOp->getLoc(),eq);
   ...
   virtual void produce(pgx::mlir::relalg::TranslatorContext& context, ::mlir::OpBuilder& builder) override {
      ...
      vector = builder.create<pgx::mlir::dsa::CreateDS>(sortOp.getLoc(), pgx::mlir::dsa::VectorType::get(builder.getContext(), tupleType));
      children[0]->produce(context,builder);
      {
         auto dbSortOp = builder.create<pgx::mlir::dsa::SortOp>(sortOp->getLoc(), vector);
         ...
         auto unpackedLeft = builder2.create<pgx::mlir::util::UnPackOp>(sortOp->getLoc(), block2->getArgument(0));
         auto unpackedRight = builder2.create<pgx::mlir::util::UnPackOp>(sortOp->getLoc(), block2->getArgument(1));
         std::vector<std::pair<mlir::Value, mlir::Value>> sortCriteria;
         for (auto attr : sortOp.sortspecs()) {
            auto sortspecAttr = attr.cast<pgx::mlir::relalg::SortSpecificationAttr>();
            ...
            if (sortspecAttr.getSortSpec() == pgx::mlir::relalg::SortSpec::desc) {
               std::swap(left, right);
            }
            sortCriteria.push_back({left, right});
         }
         auto trueVal = builder2.create<pgx::mlir::db::ConstantOp>(sortOp->getLoc(), builder.getI1Type(), builder.getIntegerAttr(builder.getI64Type(), 1));
         auto falseVal = builder2.create<pgx::mlir::db::ConstantOp>(sortOp->getLoc(), builder.getI1Type(), builder.getIntegerAttr(builder.getI64Type(), 0));

         builder2.create<pgx::mlir::dsa::YieldOp>(sortOp->getLoc(), createSortPredicate(builder2, sortCriteria, trueVal, falseVal, 0));
      }
      {
         auto forOp2 = builder.create<pgx::mlir::dsa::ForOp>(sortOp->getLoc(), ::mlir::TypeRange{}, vector, mlir::Value(), ::mlir::ValueRange{});
         ::mlir::Block* block2 = new ::mlir::Block;
         block2->addArgument(tupleType, sortOp->getLoc());
         forOp2.getBodyRegion().push_back(block2);
         ::mlir::OpBuilder builder2(forOp2.getBodyRegion());
         auto unpacked = builder2.create<pgx::mlir::util::UnPackOp>(sortOp->getLoc(), forOp2.getInductionVar());
         orderedAttributes.setValuesForColumns(context, scope, unpacked.getResults());
         consumer->consume(this, builder2, context);
         builder2.create<pgx::mlir::dsa::YieldOp>(sortOp->getLoc(), ::mlir::ValueRange{});
      }
      builder.create<pgx::mlir::dsa::FreeOp>(sortOp->getLoc(), vector);
   }

   virtual ~SortTranslator() {}
};

std::unique_ptr<pgx::mlir::relalg::Translator> pgx::mlir::relalg::createSortTranslator(::mlir::Operation* op) {
   auto sortOp = ::mlir::cast<pgx::mlir::relalg::SortOp>(op);

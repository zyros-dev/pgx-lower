...
class AggregationTranslator : public Translator {
   ...
   OrderedAttributes val;

   std::vector<std::function<std::pair<const Column*, ::mlir::Value>(::mlir::ValueRange, ::mlir::OpBuilder& builder)>> finalizeFunctions;
   ...
   public:
   AggregationTranslator(AggregationOp aggregationOp) : Translator(aggregationOp), aggregationOp(aggregationOp) {
   }
   ...
   virtual ColumnSet getAvailableColumns() override {
      ...
      for (auto& col : aggregationOp.getGroupByCols()) {
         available.insert(col.cast<::mlir::Attribute>().cast<ColumnRefAttr>().getColumn());
      }
      for (auto& col : aggregationOp.getComputedCols()) {
         available.insert(&col.cast<ColumnDefAttr>().getColumn());
      }
      return available;
   }
   ...
   ::mlir::Value compareKeys(::mlir::OpBuilder& rewriter, ::mlir::Value left, ::mlir::Value right,::mlir::Location loc) {
      ...
      auto leftUnpacked = rewriter.create<util::UnPackOp>(loc, left);
      auto rightUnpacked = rewriter.create<util::UnPackOp>(loc, right);
      for (size_t i = 0; i < leftUnpacked.getNumResults(); i++) {
         ...
         auto currLeftNullableType = currLeftType.dyn_cast_or_null<db::NullableType>();
         auto currRightNullableType = currRightType.dyn_cast_or_null<db::NullableType>();
         if (currLeftNullableType || currRightNullableType) {
            ::mlir::Value isNull1 = rewriter.create<db::IsNullOp>(loc, rewriter.getI1Type(), leftUnpacked->getResult(i));
            ::mlir::Value isNull2 = rewriter.create<db::IsNullOp>(loc, rewriter.getI1Type(), rightUnpacked->getResult(i));
            ...
            auto ifOp = rewriter.create<::mlir::scf::IfOp>(loc, rewriter.getI1Type(), anyNull, /*withElseRegion=*/true);
            {
               ...
               ::mlir::Value left = rewriter.create<db::NullableGetVal>(loc, leftUnpacked->getResult(i));
               ::mlir::Value right = rewriter.create<db::NullableGetVal>(loc, rightUnpacked->getResult(i));
               ::mlir::Value cmpRes = rewriter.create<db::CmpOp>(loc, db::DBCmpPredicate::eq, left, right);
               rewriter.create<::mlir::scf::YieldOp>(loc, cmpRes);
            }
            compared = ifOp.getResult(0);
         } else {
            compared = rewriter.create<db::CmpOp>(loc, db::DBCmpPredicate::eq, leftUnpacked->getResult(i), rightUnpacked.getResult(i));
         }
   ...
   virtual void consume(Translator* child, ::mlir::OpBuilder& builder, TranslatorContext& context) override {
      ...
      auto reduceOp = builder.create<dsa::HashtableInsert>(aggregationOp->getLoc(), aggrHt, packedKey, packedVal);
      ...
      if (!keyTupleType.getTypes().empty()) {
         {
            ...
            auto yieldOp = builder.create<dsa::YieldOp>(aggregationOp->getLoc());
            builder.setInsertionPointToStart(aggrBuilderBlock);
            ...
            builder.create<dsa::YieldOp>(aggregationOp->getLoc(), matches);
            yieldOp.erase();
         }
         {
            ::mlir::Block* aggrBuilderBlock = new ::mlir::Block;
            ...
            ::mlir::OpBuilder::InsertionGuard guard(builder);
            builder.setInsertionPointToStart(aggrBuilderBlock);
            ::mlir::Value hashed = builder.create<db::Hash>(aggregationOp->getLoc(), builder.getIndexType(), aggrBuilderBlock->getArgument(0));
            builder.create<dsa::YieldOp>(aggregationOp->getLoc(), hashed);
         }
      }
      ::mlir::Block* aggrBuilderBlock = new ::mlir::Block;
      ...
      auto unpackedCurr = builder2.create<util::UnPackOp>(aggregationOp->getLoc(), aggrBuilderBlock->getArgument(0))->getResults();
      ::mlir::ValueRange unpackedNew;
      if (valTupleType.getTypes().size() > 0) {
         unpackedNew = builder2.create<util::UnPackOp>(aggregationOp->getLoc(), aggrBuilderBlock->getArgument(1)).getResults();
      }
      ...
      ::mlir::Value packedx = builder2.create<util::PackOp>(aggregationOp->getLoc(), valuesx);

      builder2.create<dsa::YieldOp>(aggregationOp->getLoc(), packedx);
   }
   ...
   void analyze(::mlir::OpBuilder& builder) {
      key = OrderedAttributes::fromRefArr(aggregationOp.group_by_colsAttr());

      auto counterType = builder.getI64Type();
      ReturnOp terminator = ::mlir::cast<ReturnOp>(aggregationOp.aggr_func().front().getTerminator());

      for (size_t i = 0; i < aggregationOp.getComputedCols().size(); i++) {
         auto* destAttr = &aggregationOp.getComputedCols()[i].cast<ColumnDefAttr>().getColumn();
         ::mlir::Value computedVal = terminator.results()[i];
         if (auto aggrFn = ::mlir::dyn_cast_or_null<AggrFuncOp>(computedVal.getDefiningOp())) {
            auto loc = aggrFn->getLoc();
            auto* attr = &aggrFn.getAttr().getColumn();
            auto attrIsNullable = attr->type.isa<db::NullableType>();
            size_t currValIdx = val.insert(attr);
            ::mlir::Type resultingType = destAttr->type;
            size_t currDestIdx = aggrTypes.size();

            if (aggrFn.fn() == AggrFunc::sum) {
               ...
               ::mlir::Value initVal;
               if (resultingType.isa<db::NullableType>()) {
                  initVal = builder.create<db::NullOp>(loc, resultingType);
               } else {
                  initVal = builder.create<db::ConstantOp>(loc, getBaseType(resultingType), builder.getI64IntegerAttr(0));
               }
               ...
               aggregationFunctions.push_back([loc, currDestIdx = currDestIdx, currValIdx = currValIdx, attrIsNullable, resultingType = resultingType](::mlir::ValueRange aggr, ::mlir::ValueRange val, ::mlir::OpBuilder& builder) {
                  ...
                  ::mlir::Value added = builder.create<db::AddOp>(loc, resultingType, currVal, newVal);
                  ::mlir::Value updatedVal = added;
                  if (attrIsNullable) {
                     ::mlir::Value isNull1 = builder.create<db::IsNullOp>(loc, builder.getI1Type(), newVal);
                  ...
                  if (resultingType.isa<db::NullableType>()) {
                     ...
                     if (currVal.getType() != newVal.getType()) {
                        casted = builder.create<db::AsNullableOp>(loc, currVal.getType(), newVal);
                     }
                     ::mlir::Value isNull = builder.create<db::IsNullOp>(loc, builder.getI1Type(), currVal);
            ...
            } else if (aggrFn.fn() == AggrFunc::min) {
               finalizeFunctions.push_back([currDestIdx = currDestIdx, destAttr = destAttr](::mlir::ValueRange range, ::mlir::OpBuilder& builder) { return std::make_pair(destAttr, range[currDestIdx]); });
               aggrTypes.push_back(resultingType);
               ::mlir::Value initVal;
               if (resultingType.isa<db::NullableType>()) {
                  initVal = builder.create<db::NullOp>(aggregationOp.getLoc(), resultingType);
               } else {
                  initVal = builder.create<db::ConstantOp>(aggregationOp.getLoc(), getBaseType(resultingType), getMaxValueAttr(resultingType));
               }
               defaultValues.push_back(initVal);
               aggregationFunctions.push_back([loc, currDestIdx = currDestIdx, resultingType = resultingType, currValIdx = currValIdx](::mlir::ValueRange aggr, ::mlir::ValueRange val, ::mlir::OpBuilder& builder) {
                  ...
                  ::mlir::Value currVal = aggr[currDestIdx];
                  ::mlir::Value newVal = val[currValIdx];
                  ::mlir::Value newLtCurr = builder.create<db::CmpOp>(loc, db::DBCmpPredicate::lt, newVal,currVal);
                  ::mlir::Value casted = newVal;
                  if (newVal.getType() != currVal.getType()) {
                     casted = builder.create<db::AsNullableOp>(loc, currVal.getType(), newVal);
                  }
                  ::mlir::Value newLtCurrT = builder.create<db::DeriveTruth>(loc, newLtCurr);
            ...
            } else if (aggrFn.fn() == AggrFunc::max) {
               finalizeFunctions.push_back([currDestIdx = currDestIdx, destAttr = destAttr](::mlir::ValueRange range, ::mlir::OpBuilder& builder) { return std::make_pair(destAttr, range[currDestIdx]); });
               aggrTypes.push_back(resultingType);
               ::mlir::Value initVal;
               if (resultingType.isa<db::NullableType>()) {
                  ...
                  initVal = builder.create<db::ConstantOp>(aggregationOp.getLoc(), getBaseType(resultingType), builder.getI64IntegerAttr(0));
               }
               defaultValues.push_back(initVal);
               aggregationFunctions.push_back([loc, currDestIdx = currDestIdx, attrIsNullable, resultingType = resultingType, currValIdx = currValIdx](::mlir::ValueRange aggr, ::mlir::ValueRange val, ::mlir::OpBuilder& builder) {
                  ...
                  ::mlir::Value currVal = aggr[currDestIdx];
                  ::mlir::Value newVal = val[currValIdx];
                  ::mlir::Value currGtNew = builder.create<db::CmpOp>(loc, db::DBCmpPredicate::gt, currVal, newVal);
                  ::mlir::Value casted = newVal;
                  if (newVal.getType() != currVal.getType()) {
                     casted = builder.create<db::AsNullableOp>(loc, currVal.getType(), newVal);
                  }
                  ::mlir::Value currGTNewT = builder.create<db::DeriveTruth>(loc, currGtNew);
                  ...
                  if (resultingType.isa<db::NullableType>()) {
                     ::mlir::Value isNull = builder.create<db::IsNullOp>(loc, builder.getI1Type(), aggr[currDestIdx]);
                     res.push_back(builder.create<::mlir::arith::SelectOp>(loc, isNull, casted, updatedVal));
                  } else {
                     res.push_back(updatedVal);
                  }
            ...
            } else if (aggrFn.fn() == AggrFunc::avg) {
               aggrTypes.push_back(resultingType);
               aggrTypes.push_back(counterType);
               ::mlir::Value initVal = builder.create<db::ConstantOp>(aggregationOp.getLoc(), getBaseType(resultingType), builder.getI64IntegerAttr(0));
               ::mlir::Value initCounterVal = builder.create<db::ConstantOp>(aggregationOp.getLoc(), counterType, builder.getI64IntegerAttr(0));
               ::mlir::Value defaultVal = resultingType.isa<db::NullableType>() ? builder.create<db::AsNullableOp>(aggregationOp.getLoc(), resultingType, initVal) : initVal;
               ...
               finalizeFunctions.push_back([loc, currDestIdx = currDestIdx, destAttr = destAttr, resultingType = resultingType](::mlir::ValueRange range, ::mlir::OpBuilder builder) {
                  ::mlir::Value casted=builder.create<db::CastOp>(loc, getBaseType(resultingType), range[currDestIdx+1]);
                  if(resultingType.isa<db::NullableType>()&&casted.getType()!=resultingType){
                     casted=builder.create<db::AsNullableOp>(loc, resultingType, casted);
                  }
                  ::mlir::Value average=builder.create<db::DivOp>(loc, resultingType, range[currDestIdx], casted);
               ...
               aggregationFunctions.push_back([loc, currDestIdx = currDestIdx, currValIdx = currValIdx, attrIsNullable, resultingType = resultingType, counterType = counterType](::mlir::ValueRange aggr, ::mlir::ValueRange val, ::mlir::OpBuilder& builder) {
                  std::vector<mlir::Value> res;
                  auto one = builder.create<db::ConstantOp>(loc, counterType, builder.getI64IntegerAttr(1));
                  ::mlir::Value added1 = builder.create<db::AddOp>(loc, resultingType, aggr[currDestIdx], val[currValIdx]);
                  ::mlir::Value added2 = builder.create<db::AddOp>(loc, counterType, aggr[currDestIdx + 1], one);
                  if (attrIsNullable) {
                     ::mlir::Value isNull1 = builder.create<db::IsNullOp>(loc, builder.getI1Type(), val[currValIdx]);
            ...
            } else if (aggrFn.fn() == AggrFunc::count) {
               size_t currDestIdx = aggrTypes.size();
               auto initCounterVal = builder.create<db::ConstantOp>(aggregationOp.getLoc(), counterType, builder.getI64IntegerAttr(0));
               defaultValues.push_back(initCounterVal);
               aggrTypes.push_back(resultingType);
               finalizeFunctions.push_back([currDestIdx = currDestIdx, destAttr = destAttr](::mlir::ValueRange range, ::mlir::OpBuilder& builder) { return std::make_pair(destAttr, range[currDestIdx]); });
               ...
               aggregationFunctions.push_back([loc, currDestIdx = currDestIdx, attrIsNullable, currValIdx = currValIdx, counterType = counterType, resultingType = resultingType](::mlir::ValueRange aggr, ::mlir::ValueRange val, ::mlir::OpBuilder& builder) {
                  std::vector<mlir::Value> res;
                  auto one = builder.create<db::ConstantOp>(loc, counterType, builder.getI64IntegerAttr(1));
                  ::mlir::Value value = builder.create<db::AddOp>(loc, resultingType, aggr[currDestIdx], one);
                  if (attrIsNullable) {
                     ::mlir::Value isNull2 = builder.create<db::IsNullOp>(loc, builder.getI1Type(), val[currValIdx]);
            ...
            } else if (aggrFn.fn() == AggrFunc::any) {
               size_t currDestIdx = aggrTypes.size();
               auto initVal = builder.create<util::UndefOp>(aggregationOp.getLoc(), resultingType);
               defaultValues.push_back(initVal);
               aggrTypes.push_back(resultingType);
               finalizeFunctions.push_back([currDestIdx = currDestIdx, destAttr = destAttr](::mlir::ValueRange range, ::mlir::OpBuilder& builder) { return std::make_pair(destAttr, range[currDestIdx]); });
         ...
         if (auto countOp = mlir::dyn_cast_or_null<relalg::CountRowsOp>(computedVal.getDefiningOp())) {
            ...
            aggregationFunctions.push_back([loc, currDestIdx = currDestIdx, counterType = counterType](::mlir::ValueRange aggr, ::mlir::ValueRange val, ::mlir::OpBuilder& builder) {
               std::vector<mlir::Value> res;
               auto one = builder.create<db::ConstantOp>(loc, counterType, builder.getI64IntegerAttr(1));
               ::mlir::Value added2 = builder.create<db::AddOp>(loc, counterType, aggr[currDestIdx], one);
               res.push_back(added2);
               return res;
            });
         }
   ...
   virtual void produce(relalg::TranslatorContext& context, ::mlir::OpBuilder& builder) override {
      auto scope = context.createScope();
         analyze(builder);
         auto aggrTupleType = mlir::TupleType::get(builder.getContext(), aggrTypes);
         auto initTuple = builder.create<util::PackOp>(aggregationOp->getLoc(), aggrTupleType, defaultValues);
         keyTupleType = key.getTupleType(builder.getContext());
         valTupleType = val.getTupleType(builder.getContext());
         aggrHt = builder.create<dsa::CreateDS>(aggregationOp.getLoc(), dsa::AggregationHashtableType::get(builder.getContext(), keyTupleType, aggrTupleType), initTuple);

      auto iterEntryType = mlir::TupleType::get(builder.getContext(), {keyTupleType, aggrTupleType});
      children[0]->produce(context, builder);

      {
         auto forOp2 = builder.create<dsa::ForOp>(aggregationOp->getLoc(), ::mlir::TypeRange{}, aggrHt, mlir::Value(), ::mlir::ValueRange{});
         ::mlir::Block* block2 = new ::mlir::Block;
         block2->addArgument(iterEntryType, aggregationOp->getLoc());
         forOp2.getBodyRegion().push_back(block2);
         ::mlir::OpBuilder builder2(forOp2.getBodyRegion());
         auto unpacked = builder2.create<util::UnPackOp>(aggregationOp->getLoc(), forOp2.getInductionVar()).getResults();
         ::mlir::ValueRange unpackedKey;
         if (!keyTupleType.getTypes().empty()) {
            unpackedKey = builder2.create<util::UnPackOp>(aggregationOp->getLoc(), unpacked[0]).getResults();
         }
         auto unpackedAggr = builder2.create<util::UnPackOp>(aggregationOp->getLoc(), unpacked[1]).getResults();
         ...
         builder2.create<dsa::YieldOp>(aggregationOp->getLoc(), ::mlir::ValueRange{});
      }
      builder.create<dsa::FreeOp>(aggregationOp->getLoc(), aggrHt);
   }
...
std::unique_ptr<pgx::mlir::relalg::Translator> pgx::mlir::relalg::Translator::createAggregationTranslator(AggregationOp aggregationOp) {
   auto sortOp = ::mlir::cast<pgx::mlir::relalg::AggregationOp>(op);

...
SELECT id, small_int, integer_col, big_int
FROM test;

SELECT id, real_col, double_col
FROM test;

SELECT id, boolean_col
FROM test;

SELECT id, char_col, varchar_col, text_col
FROM test;

SELECT id, decimal_col, numeric_col, money_col
FROM test;

SELECT id, date_col, time_col, timetz_col
FROM test;

SELECT id, timestamp_col, timestamptz_col, interval_col
FROM test;

SELECT id, bytea_col, uuid_col
FROM test;

SELECT id, inet_col, cidr_col, macaddr_col
FROM test;

SELECT id, bit_col, varbit_col
FROM test;

FINAL: Test all types together (original Test 6 goal)

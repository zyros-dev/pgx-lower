...
TEST_F(MLIRLoweringPipelineTest, TestConstantOp) {
    // Test the ACTUAL failing MLIR from PostgreSQL regression test
    auto actualFailingMLIR = R"(
        module {
          func.func @main() {
            %0 = db.constant("hello") : !db.char<5>
            %1 = relalg.const_relation columns : [@const::@"?column?"({type = !db.char<5>})=[]] values : [["hello"]]
            %2 = relalg.materialize %1 [@const::@"?column?"] => ["?column?"] : !dsa.table
            return
          }
        }
    )";

    ASSERT_TRUE(tester->loadRelAlgModule(actualFailingMLIR)) << "Failed to load MLIR module";

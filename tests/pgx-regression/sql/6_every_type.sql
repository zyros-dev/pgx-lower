...
/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_6_every_type_001 */
SELECT id, integer_value, big_value
FROM test;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_6_every_type_002 */
SELECT category_name, text_field
FROM test;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_6_every_type_003 */
SELECT id, boolean_flag, text_field
FROM test;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_6_every_type_004 */

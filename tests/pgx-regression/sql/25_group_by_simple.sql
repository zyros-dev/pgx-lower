...
/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_25_group_by_simple_001 */
SELECT quantity, COUNT(*) AS item_count
FROM simple_group_by
GROUP BY quantity;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_25_group_by_simple_002 */
SELECT quantity, COUNT(*) AS item_count
FROM simple_group_by
GROUP BY quantity
ORDER BY quantity;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_25_group_by_simple_003 */
SELECT quantity, COUNT(*) AS item_count
FROM simple_group_by
GROUP BY quantity
ORDER BY COUNT(*);

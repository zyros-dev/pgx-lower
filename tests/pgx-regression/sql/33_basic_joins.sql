...
/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_001 */
SELECT *
FROM table_a, table_b
WHERE table_a.id = table_b.id;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_002 */
...
/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_004 */
SELECT table_a.name, table_b.amount, table_c.category
FROM table_a, table_b, table_c
WHERE table_a.id = table_b.id AND table_a.id = table_c.id;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_005 */
SELECT table_a.name, table_a.value, table_b.amount
FROM table_a, table_b
WHERE table_a.id = table_b.id AND table_a.value > table_b.amount * 5;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_006 */
SELECT table_a.name, table_b.amount
FROM table_a, table_b
WHERE table_a.id = table_b.id
ORDER BY table_b.amount DESC;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_007 */
SELECT table_a.id AS a_id, table_b.id AS b_id
FROM table_a, table_b
ORDER BY a_id, b_id;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_008 */
SELECT table_a.name, table_a.value + table_b.amount AS total
FROM table_a, table_b
WHERE table_a.id = table_b.id;

/* <<pgx-lower-config>>: auto_should_route_to=lower id=pgx_33_basic_joins_009 */

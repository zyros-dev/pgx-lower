...
TEST_F(RealModuleCrashTest, ExactRealModuleContent) {
    PGX_INFO("=== RealModuleCrashTest: Testing exact real module content ===");
    
    // The exact MLIR that crashes in production
...
module {
  ...
  func.func @main() -> i32 {
    ...
    PGX_INFO("Parsing real module content...");
    auto module = parseSourceString<ModuleOp>(moduleStr, &context);
    ASSERT_TRUE(module) << "Failed to parse real module";
    
    PGX_INFO("Module parsed successfully, creating PassManager...");
    PassManager pm(&context);
    pm.addPass(pgx_lower::createStandardToLLVMPass());
    
    PGX_INFO("Running StandardToLLVMPass on real module content...");
    
    // This should reproduce the exact crash
    bool result = succeeded(pm.run(module.get()));
    
    if (result) {
        PGX_INFO("✅ SUCCESS: Real module content converted successfully!");
    } else {
        PGX_ERROR("❌ FAILED: Real module content conversion failed");
    }

...
struct ErrorInfo {
    ...
    ErrorInfo(ErrorSeverity sev, ErrorCategory cat, const std::string& msg, 
        ...
        : severity(sev), category(cat), message(msg), context(ctx) {}
...
public:
    ...
    explicit Result(const ErrorInfo& error)
        : success_(false), value_(), error_(error) {}
...
public:
    ...
    static ErrorInfo makeError(ErrorSeverity severity, ErrorCategory category,
    ...
    static ErrorInfo queryAnalysisError(const std::string& message, 
                                       const std::string& queryText = "");
    
    static ErrorInfo mlirGenerationError(const std::string& message,
                                        const std::string& context = "");
    
    static ErrorInfo compilationError(const std::string& message,
                                     const std::string& context = "");
    
    static ErrorInfo executionError(const std::string& message,
                                   const std::string& context = "");
    
    static ErrorInfo postgresqlError(const std::string& message,
                                    const std::string& context = "");
};
...
#define MAKE_ERROR(severity, category, message) \
    pgx_lower::ErrorManager::makeError(pgx_lower::ErrorSeverity::severity, \
                                       pgx_lower::ErrorCategory::category, message)

#define MAKE_ERROR_CTX(severity, category, message, context) \
    pgx_lower::ErrorManager::makeError(pgx_lower::ErrorSeverity::severity, \
                                       pgx_lower::ErrorCategory::category, message, context)

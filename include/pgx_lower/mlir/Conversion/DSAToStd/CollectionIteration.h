...
class CollectionIterationImpl {
   public:
   virtual std::vector<Value> implementLoop(::mlir::Location loc,::mlir::ValueRange iterArgs, Value flag, const ::mlir::TypeConverter& typeConverter, ConversionPatternRewriter& builder, ::mlir::ModuleOp parentModule, std::function<std::vector<Value>(std::function<Value(OpBuilder&)>,ValueRange, OpBuilder)> bodyBuilder) = 0;

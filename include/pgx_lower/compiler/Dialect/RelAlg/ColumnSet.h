...
class ColumnSet {
   using attribute_set = llvm::SmallPtrSet<const pgx_lower::compiler::dialect::tuples::Column*, 8>;
   ...
   void insert(const pgx_lower::compiler::dialect::tuples::Column* attr) {
      attributes.insert(attr);
   }
   bool contains(const pgx_lower::compiler::dialect::tuples::Column* attr) const {
   ...
   void dump(mlir::MLIRContext* context) {
      auto& attributeManager = context->getLoadedDialect<pgx_lower::compiler::dialect::tuples::TupleStreamDialect>()->getColumnManager();
      for (const auto* x : attributes) {
   ...
   static ColumnSet fromArrayAttr(mlir::ArrayAttr arrayAttr) {
      ...
      for (const auto attr : arrayAttr) {
         if (auto attrRef = mlir::dyn_cast_or_null<pgx_lower::compiler::dialect::tuples::ColumnRefAttr>(attr)) {
            res.insert(&attrRef.getColumn());
         } else if (auto attrDef = mlir::dyn_cast_or_null<pgx_lower::compiler::dialect::tuples::ColumnDefAttr>(attr)) {
   ...
   static ColumnSet from(const pgx_lower::compiler::dialect::tuples::Column* col) {
      ColumnSet res;
      res.insert(col);
      return res;
   }

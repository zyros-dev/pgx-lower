...
- Codex CLI must trust the project `.codex/` layer for
  ...
  rules block broad opaque shell wrapper prefixes such as `bash -lc` and raw
  `cat`/`tail`; use direct non-opaque local inspection commands or typed
  `pgx-cli` commands instead. The `pgx-cli codex-policy check` classifier also
  inspects shell bodies for detailed replacement guidance. Automatic external
...
- `pgx-cli dev gate review` runs `pgx-cli dev preflight --strict` as its first
  ...
  include that successful preflight step and enforces the named
  `agent-hardening` evidence contract; arbitrary self-declared one-row evidence
  is not PR-ready evidence.
